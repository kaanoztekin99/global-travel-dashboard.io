class ParallelCoordinates {
  constructor(containerId, data) {
    ...
    initChart(data) {        
        ...
        const activeCount = cities.filter(d => d.active).length;
        ...
        const brushMoved = (event, dimension) => {
            ...
            this.cities = data.filter(d => d.active)
            window.interactionManager.publish(this.events.brush, {cities: this.cities, countries: null, heritage: null});
        };
        ...
        const lines = this.chart.selectAll(".line")
            ...
            .attr("d", path)
            .style("fill", "none")
            .style("stroke", d => isLineActive(d) && d.active ? "#7a0177" : "#999")
            .style("opacity", d => isLineActive(d) && d.active ? Math.max(1/Math.exp(.04*(activeCount-1)), 0.1) : 0.01);

        const updateLines = () => {
            lines.style("stroke", d => isLineActive(d) && d.active ? "#7a0177" : "#999")
                 .style("opacity", d => isLineActive(d) && d.active ? Math.max(1/Math.exp(.04*(activeCount-1)), 0.1) : 0.01);
        };
