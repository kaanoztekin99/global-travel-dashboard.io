...
function extendBoundsFromCoordinates(bounds, coordinates) {
  ...
  coordinates.forEach((childCoordinates) => {
    bounds = extendBoundsFromCoordinates(bounds, childCoordinates);
  });

  return bounds;
}

function focusMapCountry(countryName) {
  const map = window.dashboardMap;
  const geojson = window.countriesGeojson;
  const normalizedCountryName = normalizeMapCountryName(countryName);

  if (!map || !geojson || !normalizedCountryName) return;
  ...
  const countryFeature = geojson.features.find((feature) => {
    ...
    return candidateNames.some((candidateName) =>
      normalizeMapCountryName(candidateName) === normalizedCountryName
    );
  });

  if (!countryFeature) return;

  const bounds = extendBoundsFromCoordinates(null, countryFeature.geometry.coordinates);

  if (!bounds) return;

  if (typeof window.showDashboardMap === "function") {
    window.showDashboardMap();
  }
...
function getEmptyFocusedCityGeojson() {
  return {
...
function setFocusedCityMarker(city) {
  const map = window.dashboardMap;
  const source = map?.getSource("focused-city");
...
function clearFocusedCityMarker() {
  const map = window.dashboardMap;
  const source = map?.getSource("focused-city");

  if (source) {
    source.setData(getEmptyFocusedCityGeojson());
  }
}

window.clearFocusedCityMarker = clearFocusedCityMarker;

function focusMapCity(city) {
  const map = window.dashboardMap;
  const latitude = Number(city?.latitude);
  const longitude = Number(city?.longitude);

  if (!map || !Number.isFinite(latitude) || !Number.isFinite(longitude)) {
    focusMapCountry(city?.country);
    return;
  }
...
window.focusMapCity = focusMapCity;
...
function addCountryCodesToSelection(countryCodes) {
  ...
  window.interactionManager.publish("mapCountrySelected", window.selectedCountryCodes)
...
function initMap() {
  ...
  map.on("load", async () => {
    const response = await fetch(resolveSitePath("data/geojson/countries.geojson"));
    const geojson = await response.json();

    // load data from country arrivals dataset
    const countriesData = await fetch(resolveSitePath("data/clean/countries.csv"));
    ...
    const heritageData = await fetch(resolveSitePath("data/clean/heritage.csv"));
    ...
    geojson.features.forEach((feature, i) => {
      ...
      p.display_name =
        ...
        arrivalsData?.name ||
        `Country ${i + 1}`;
      p.arrival_name = arrivalsData?.name || null;
    ...
    map.addSource("focused-city", {
      type: "geojson",
      data: getEmptyFocusedCityGeojson()
    });

    map.addLayer({
      id: "focused-city-marker",
      type: "circle",
      source: "focused-city",
      paint: {
        "circle-color": "#dc2626",
...
function createMapLegend(map) {
  ...
  // Get min and max from the current data
  const geojson = window.countriesGeojson || { features: [] };
  const values = geojson.features
    .map((f) => f.properties.arrival_count)
    .filter((v) => v !== null && Number.isFinite(v));

  const minValue = values.length > 0 ? Math.min(...values) : 0;
  const maxValue = values.length > 0 ? Math.max(...values) : 1000000;

  legend.innerHTML = `
    <div class="map-legend-header">
      <div class="map-legend-title">International Arrivals</div>
    ...
    <div class="map-legend-gradient">
      <span class="legend-label-min">${formatNumber(minValue)}</span>
      <div class="legend-gradient-bar"></div>
      <span class="legend-label-max">${formatNumber(maxValue)}</span>
    </div>
  `;
