class CityDurationBudgetStackedBar {
  ...
  initChart(data) {
    ...
    const cities = data.cities.filter(d => d.active);
    ...
    const sampledCities = cities.filter((city) =>
        city.city &&
        city.country &&
    ...
    this.chart.selectAll("g.layer")
      ...
      .on("click", (event, d) => {
        ...
        const budgetLevel = d3.select(event.currentTarget.parentNode).datum().key;
        ...
        if (!isSelected) {
          ...
          if(this.prevTarg !== null)
            this.prevTarg.classed("selected", false);

          this.selectedData = { duration, budgetLevel, percent };
          window.interactionManager.publish(this.events.click, this.selectedData);
          this.prevTarg = target;
        }
        else {
          target.classed("selected", false)
          window.interactionManager.publish(this.events.click, null);
        }
      })
