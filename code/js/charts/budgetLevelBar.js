class BudgetLevelBarChart {
  constructor(containerId) {
    ...
    this.events = {
      countrySelected: 'budgetLevelBarCountrySelected',
      countryDeselected: 'budgetLevelBarCountryDeselected'
    }
  ...
  async initChart(data) {
    ...
    this.render(this.prepareData(cities.filter(city => city.active), countries));
  }
  ...
  render(countryRows) {
    ...
    const bars = chart.selectAll("g.budget-layer")
      .data(series)
      .enter()
      .append("g")
      ...
      .attr("stroke", (d) => d.data.country === this.selectedCountry ? "#000000" : "rgba(255, 255, 255, 0.75)")
      .attr("stroke-width", (d) => d.data.country === this.selectedCountry ? 2.5 : 1)
      ...
      .on("mouseout", (event) => {
        ...
        d3.select(event.currentTarget)
          .attr("opacity", d.data.country === this.selectedCountry ? 0.88 : 1);
      })
      .on("click", (event, d) => {
        const country = d.data.raw;
        const isSelected = this.selectedCountry === country.country;
        this.selectedCountry = isSelected ? null : country.country;
        this.selectedData = isSelected ? null : {
          country: country.country,
          budgetLevel: d.level
        };
        ...
        if (!isSelected) {
          ...
          if (charts && charts.countryHeritageScatter && typeof charts.countryHeritageScatter.clearSelection === "function") {
            charts.countryHeritageScatter.clearSelection();
          }

          if (typeof window.focusMapCountry === "function") {
            window.focusMapCountry(country.country);
          }
        }

        this.applySelection(bars);
      });
  ...
  applySelection(bars) {
    const targetBars = bars || d3.select(`#${this.containerId}`).selectAll("g.budget-layer rect");

    targetBars
      .attr("stroke", (d) => d.data.country === this.selectedCountry ? "#000000" : "rgba(255, 255, 255, 0.75)")
      .attr("stroke-width", (d) => d.data.country === this.selectedCountry ? 2.5 : 1)
      .attr("opacity", (d) => d.data.country === this.selectedCountry ? 0.88 : 1);
  }
