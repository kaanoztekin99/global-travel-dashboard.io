function App() {
  ...
  React.useEffect(() => {
    window.showDashboardMap = () => setMapMode("2d");

    return () => {
      delete window.showDashboardMap;
    };
  }, []);
    ...
    const chartInitTimer = setTimeout(() => {
      if (plotLevel === "country") {
        if (window.interactionManager){
          window.interactionManager.charts.countryHeritageScatter.initChart(window.interactionManager.data);
          window.interactionManager.charts.budgetLevelBar.initChart(window.interactionManager.data);
        }
    }

      if (plotLevel === "city") {
        window.interactionManager.charts.cityTemperatureHorizon.initChart(window.interactionManager.data);
        if (window.interactionManager.charts.cityDurationBudgetStackedBar) {
          window.interactionManager.charts.cityDurationBudgetStackedBar.initChart(window.interactionManager.data);
        }
      }
    }, 100);
  ...
  React.useEffect(() => {
        ...
        Promise.all([countryDataPromise, cityDataPromise, heritageDataPromise])
            .then(([countryData, cityData, heritageData]) => {
              const charts = {
                parallelCoordinates: new ParallelCoordinates("mos-parallel-coordinates"),
                cityDurationBudgetStackedBar: new CityDurationBudgetStackedBar("city-duration-budget-stacked"),
                countryHeritageScatter: new CountryHeritageScatter("country-heritage-scatter"),
                budgetLevelBar: new BudgetLevelBarChart("budget-level-bar"),
                cityTemperatureHorizon: new CityTemperatureHorizonChart("city-temperature-violin"),
              }
              const data = {cities: cityData, countries: countryData, heritage: heritageData};
              window.interactionManager = new InteractionManager(data, charts);

              charts.parallelCoordinates.initChart(data);
            });
  }, []);
      ...
      <div className="dashboard-body">
        ...
        <main className="main-content">
          ...
          <section className="charts-grid">
            ...
            {plotLevel === "country" ? (
              <React.Fragment key="country-level-charts">
              ...
              <React.Fragment key="city-level-charts">
                <div className="chart-card">
